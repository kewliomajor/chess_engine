...
public class ByteBoard extends AbstractBoard<ByteBoard>{
    ...
    private PieceScoreBoard pieceScoreBoard = new PieceScoreBoard();

    @Override
    public ByteBoard getInstance(AbstractBoard board) {
        return new ByteBoard((ByteBoard) board);
    }

    @Override
    public ByteBoard getInstance(Color playerColor) {
        return new ByteBoard(playerColor);
    }

    /**
     * Creates a board with all pieces in the starting formation
     */
    public ByteBoard(Color playerColor){
    ...
    public ByteBoard(Color playerColor, boolean debug){
    ...
    public ByteBoard(ByteBoard bitBoard){
    ...
    private double getPieceValue(byte piece, int position){
        ...
        if (BitPieces.isPieceKing(piece)){
            ...
            value = 9;
            value += pieceScoreBoard.getScoreOfPiece(PieceScoreBoard.PieceType.QUEEN, position);
        }
        else if (BitPieces.isPieceRook(piece)){
            value = 5;
            value += pieceScoreBoard.getScoreOfPiece(PieceScoreBoard.PieceType.ROOK, position);
        }
        else if (BitPieces.isPieceKnight(piece)){
            value = 3;
            value += pieceScoreBoard.getScoreOfPiece(PieceScoreBoard.PieceType.KNIGHT, position);
        }
        else if (BitPieces.isPieceBishop(piece)){
            value = 3;
            value += pieceScoreBoard.getScoreOfPiece(PieceScoreBoard.PieceType.BISHOP, position);
        }
        else if (BitPieces.isPiecePawn(piece)){
            value = 1;
            value += pieceScoreBoard.getScoreOfPiece(PieceScoreBoard.PieceType.PAWN, position);
        }
        else{
    ...
    private boolean isKingMoveValid(int fromPiece, int toPiece){
        //case for castling
        Move move;
        ByteBoard afterMoveBoard;
        if (fromPiece+2 == toPiece){
            afterMoveBoard = new ByteBoard(this);
